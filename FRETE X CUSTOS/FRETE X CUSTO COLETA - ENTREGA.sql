select DISTINCT
	...
	,( CASE
		...
		END) AS FRETE_RATEIO 
	,( CASE
		WHEN VCC.DEB_CRED_TIPO_PAGTO = 'C' THEN (Frete_Cte.PESO_MOV/VCC_COUNT.QTD_DEB_CRED_TIPO_PAGTO) /1000
		END) AS PESO_MOV_RATEIO_TON 

	,Frete_Cte.FRETE
	,QTD_DEB_CRED_TIPO_PAGTO
	FROM
		(
			select DISTINCT  dmce.NUM_CONTRATO, DMCE.FIL_CONTRATO ,count(DMCE.NUM_DOC) as NUM_DOCs, sum(DRECE.frete) as FRETE, SUM(DRECE.PESO_MOV)/100 AS PESO_MOV from tb_dados_rom_coleta_custo_emis  as DMCE
			left  JOIN
