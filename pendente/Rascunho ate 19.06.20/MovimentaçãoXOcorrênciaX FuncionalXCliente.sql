...
SELECT DISTINCT
	...
	,	O.CODIGO_OCOR
	, ( CASE WHEN O.FALHA_CLIENTE = 'S' THEN 'CLIENTE' ELSE 'TSV' END) AS RESPONSAVEL_OCOR
	,	M.*	
FROM 	tb_dados_movimento_custo_emis AS M	 						
	...
	AND
	(
	M.DT_EMISSAO >= DATEADD(DAY,-1,GETDATE())
	OR
	M.DT_ATUALIZADO  >= DATEADD(DAY,-1,GETDATE())
	OR 
	O.DATA_OCORRENCIA >= DATEADD(DAY,-1,GETDATE())
	OR
	M.DT_ENTREGA > = DATEADD(DAY,-1,GETDATE())
	)
	AND
	...
	(	SELECT  	MAX(ODM.DATA_OCORRENCIA)				
		FROM	tb_solid_ocorrencias AS ODM				
			...
			AND
			(
				MDM.DT_EMISSAO >= DATEADD(DAY,-1,GETDATE())
				OR
				MDM.DT_ATUALIZADO  >= DATEADD(DAY,-1,GETDATE())
				OR 
				ODM.DATA_OCORRENCIA >= DATEADD(DAY,-1,GETDATE())
				OR
				MDM.DT_ENTREGA >= DATEADD(DAY,-1,GETDATE())
			)
...
FROM	tb_dados_movimento_custo_emis AS MNF						
	...
	AND						
	(
				MNF.DT_EMISSAO >= DATEADD(DAY,-1,GETDATE())
				OR
				MNF.DT_ATUALIZADO  >= DATEADD(DAY,-1,GETDATE())
			)				
) MONF
LEFT JOIN tb_solid_dados_cliente AS CL
ON  MONF.CGC_CONSIG = CL.CGC

ORDER BY MONF.DATA_OCORRENCIA DESC