...
SELECT DISTINCT
	...
	FROM	
		...
		FROM
			(SELECT DISTINCT  
				  dbo.fn_ret_notas_cte(DMCE01.NR_ENTRADA, DMCE01.SIGLA_FIL) AS NOTA_AGRUP 
				, DMCE01.*
				,(CASE WHEN DATEPART(DW,PREVISAO_ENTREGA) = 7  THEN  DATEADD(DD,-1,PREVISAO_ENTREGA) ELSE PREVISAO_ENTREGA END) AS PREVISAO_2 
			FROM   tb_dados_movimento_custo_emis AS DMCE01 WHERE DMCE01.TIPO_DOC IN('NF','CO') and DMCE01.PREVISAO_ENTREGA BETWEEN '20200101' AND dateadd(DD,-1,GETDATE())) DMC02
		LEFT JOIN 
	...
	LEFT JOIN
		...
		ON TABELA.CIDADE_DESTINO = PRAZO.CIDADE AND TABELA.NOME_PROP = PRAZO.NOME_PARCEIRO
			where
			(TABELA.DT_EMISSAO > dateadd(DD,-45,GETDATE())
			or TABELA.DT_ENTREGA > dateadd(DD,-45,GETDATE()))
