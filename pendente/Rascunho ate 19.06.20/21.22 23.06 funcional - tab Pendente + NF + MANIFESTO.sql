SELECT DISTINCT
	 (CASE 
		WHEN MONFCLMNF.STATUS_ENTREGA = 'EM_ATRASO' THEN MONFCLMNF.NUM_DOC
		END) AS DOC_ATRASADO
	...
	,(CASE
		...
		WHEN ENT.CPF_PROPRIETARIO IS NOT NULL THEN pl.PLACA_PRINCIPAL
		WHEN MNFE.NRO_MANIFESTO IS NULL THEN MNFE.NOME_PROP
		ELSE
		'0'
      END) AS  NOME_PROP
	,(CASE
		WHEN RDPC.RAZAO IS NOT NULL THEN RDPC.CGC_REDESPACHO
		WHEN ENT.CPF_PROPRIETARIO IS NOT NULL THEN ENT.CPF_PROPRIETARIO
		WHEN MNFE.NRO_MANIFESTO IS NULL THEN MNFE.CPF_PROPRIETARIO
		ELSE
		'0'
      END)  AS  CPF_CNPJ
	,(CASE
		WHEN RDPC.RAZAO IS NOT NULL THEN 'REDESPACHO'
		WHEN ENT.CPF_PROPRIETARIO IS NOT NULL THEN 'ENTREGA'
		WHEN MNFE.NRO_MANIFESTO IS NULL THEN 'FIL ORIGEM'
		WHEN MNFE.DT_CHEGADA IS NULL THEN 'TRASFERENCIA'
...
FROM 
	...
	FROM
		(SELECT DISTINCT
			...
			FROM
				(SELECT DISTINCT
					...
					,O.CODIGO_OCOR
					,(CASE
						WHEN CODIGO_OCOR IN(1,2,62,76,84,94,100,117,119,120,121,301,303) AND M.DT_ENTREGA IS NULL THEN 'SISTEMA'
					...
					,(CASE 	
						...
						WHEN CODIGO_OCOR IN(1,43,49,52,67,89,94,98,101,300,302,25,102) THEN 'CONTROL TOWER'
					...
					, M.*
				FROM 	
					tb_dados_movimento_custo_emis AS M
				...
				SELECT NULL,'NF DE SERVIÇO',0, 'NF DE SERVIÇO', 'NF DE SERVIÇO','NF DE SERVIÇO',  MNF.*
		...
		WHERE
			MONFCL.DT_ENTREGA is  null) AS MONFCLMNF

			LEFT JOIN
	tb_dados_manifesto_custo_emis AS MNFE
	ON 	concat(cast(MONFCLMNF.NR_ENTRADA as char) , MONFCLMNF.SIGLA_FIL) = concat(cast(MNFE.NR_ENTRADA as CHAR), MNFE.SIGLA_FIL)
LEFT JOIN 
	tb_valores_redespacho_custo_em AS  RDPC
	ON 	concat(cast(MONFCLMNF.NR_ENTRADA as char) , MONFCLMNF.SIGLA_FIL) = concat(cast(RDPC.NR_ENTRADA as CHAR), RDPC.SIGLA_FIL)
LEFT JOIN
	tb_dados_rom_entrega_custo_em AS ENT
	ON 	concat(cast(MONFCLMNF.NR_ENTRADA as char) , MONFCLMNF.SIGLA_FIL) = concat(cast(ENT.NR_ENTRADA as CHAR), ENT.SIGLA_FIL)
LEFT JOIN Tb_valores_contrato_custo AS PL
	ON 	concat(cast(ENT.NUM_CONTRATO AS char) , ENT.FIL_CONTRATO) = concat(cast(PL.NUM_CONTRATO as CHAR), PL.FIL_CONTRATO)
	  WHERE
		 MNFE.DT_EMISSAO_MANIFESTO = 
			(
				SELECT MAX(MNFDX.DT_EMISSAO_MANIFESTO) FROM tb_dados_manifesto_custo_emis AS MNFDX
				WHERE 
						MNFDX.NR_ENTRADA = 	MNFE.NR_ENTRADA
