	...
	FROM	
		...
		FROM
			...
			WHERE DMCE01.TIPO_DOC IN('NF','CO') 
				and DMCE01.PREVISAO_ENTREGA BETWEEN dateadd(DD,-30,GETDATE()) AND dateadd(DD,-1,GETDATE())
			) DMC02
		LEFT JOIN 
		...
		LEFT JOIN
			(SELECT DISTINCT PLACA_PRINCIPAL, FIL_CONTRATO ,NUM_CONTRATO FROM Tb_valores_contrato_custo with(nolock) WHERE DT_EMIS_CONTRATO > '20200101' ) AS VCC
