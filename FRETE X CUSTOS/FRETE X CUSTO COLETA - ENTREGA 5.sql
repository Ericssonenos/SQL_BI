...
entrega,
 (CASE 
	when cast((peso_coleta)as float) > cast((peso_coleta_C)as float)
	then cast((peso_coleta)as float) else cast((peso_coleta_C)as float)
End) as peso_col,
 (CASE 
	when cast((peso_entrega)as float) > cast((peso_entrega_C)as float)
	then cast((peso_entrega)as float) else cast((peso_entrega_C)as float)
End) as peso_ent,
...
from tb_valores_contrato_custo vcc with(nolock) 	
	...
	outer apply (select  
					(case 
						when sum((case when rpd.ds_tipo = 'COLETA' then  nr_peso_rel else 0 end)) > sum((case when rpd.ds_tipo = 'COLETA' then  PESO_CUBADO else 0 end))
						      then sum((case when rpd.ds_tipo = 'COLETA' then  nr_peso_rel else 0 end)) else  sum((case when rpd.ds_tipo = 'COLETA' then  PESO_CUBADO else 0 end) )
					end) as peso_coleta ,
					(case 
						when sum((case when rpd.ds_tipo = 'ENTREGA' then  nr_peso_rel else 0 end)) > sum((case when rpd.ds_tipo = 'ENTREGA' then  PESO_CUBADO else 0 end))
						      then sum((case when rpd.ds_tipo = 'ENTREGA' then  nr_peso_rel else 0 end)) else  sum((case when rpd.ds_tipo = 'ENTREGA' then  PESO_CUBADO else 0 end) )
					end) as peso_entrega ,
				from tb_rel_romaneio_prop_dados rpd  with(nolock) 	
				where
