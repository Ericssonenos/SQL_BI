...
O.DATA_OCORRENCIA, 
(CASE WHEN O.RESPONSAVEL = 'AUTOMATICO' THEN 'NÃO TRATADO' WHEN O.RESPONSAVEL IS NULL THEN 'NÃO TRATADO' ELSE O.DESCRICAO_OCOR END) AS  OCORRENCIA 
...
AND
(
M.DT_EMISSAO >= DATEADD(DAY , -12 , GETDATE())
OR
M.DT_ENTREGA >= DATEADD(DAY , -12 , GETDATE())
OR
M.DATA_COLETA >= DATEADD(DAY , -12 , GETDATE())
OR
O.DATA_OCORRENCIA > = DATEADD(DAY , -12 , GETDATE())
)	
...
MNF.DT_EMISSAO >= DATEADD(DAY , -12 , GETDATE())
OR
MNF.DT_ENTREGA >= DATEADD(DAY , -12 , GETDATE())
)) MONF 
