SELECT DISTINCT 
	...
	,(CASE 
		WHEN TABELA.CODIGO_OCOR IN(79,311,89,3,9,26,30,46,82,89,100) THEN 'Pendência ADM'
		WHEN TABELA.DIAS_ATRASO > 0 THEN 'ATRASADO' 
		when TABELA.DIAS_ATRASO = 0 and DATEPART(dw,  TABELA.PREVISAO_ENTREGA) = 7 then 'ATRASADO' 
	...
	FROM	
			...
			,(CASE 
			    WHEN SO03.CODIGO_OCOR IN(321,307) THEN SO03.DESCRICAO_OCOR
				WHEN SO03.CODIGO_OCOR IN(79,311,89,3,9,26,30,46,82,89,100) THEN 'Pendência ADM'
				WHEN DMC02.CONSIGNATARIO IN('DISTRIBUIDORA AMARAL LTDA','B L INDUSTRIA OTICA LTDA', 'DOARBELLEZA PRODUTOS DE BELEZA','DOARBELLEZA PRODUTOS DE BELEZA- FILIAL E') and Agen3.DESCRICAO_OCOR IS NULL  THEN '24 HORAS'
			...
			,(CASE 
				...
				WHEN DMC02.SIGLA_FIL_DEST IN('BNU','ITJ','FLN') THEN 'BNU'
		...
		FROM
			...
			AND DMCE01.DT_EMISSAO > '20200301'
		...
		LEFT JOIN 
			(SELECT * FROM tb_solid_ocorrencias AS SO01  with(nolock)
										WHERE SO01.DATA_OCORRENCIA = (SELECT MAX(SO02.DATA_OCORRENCIA) FROM tb_solid_ocorrencias AS SO02 WHERE SO02.NR_ENTRADA = SO01.NR_ENTRADA AND SO02.SIGLA_FIL = SO01.SIGLA_FIL ) 
		...
		LEFT JOIN 
			(SELECT * FROM tb_dados_manifesto_custo_emis AS DMFCE01  with(nolock)
										WHERE DMFCE01.DT_EMISSAO_MANIFESTO = (SELECT MAX(DMFCE02.DT_EMISSAO_MANIFESTO) FROM tb_dados_manifesto_custo_emis AS DMFCE02 WHERE DMFCE02.NR_ENTRADA = DMFCE01.NR_ENTRADA AND DMFCE02.SIGLA_FIL = DMFCE01.SIGLA_FIL ) 
		...
		LEFT JOIN
			(SELECT DISTINCT PLACA_PRINCIPAL, FIL_CONTRATO ,NUM_CONTRATO , NOME_MOT FROM Tb_valores_contrato_custo  with(nolock) ) AS VCC
