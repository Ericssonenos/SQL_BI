	 ...
	 ,(CASE 		
		...
		,MONFCLMNF.* FROM
			...
			ON MONFCL.CGC_CONSIG = SAC.nr_cnpj_cpf 
			  OUTER APPLY ( 
						SELECT (  LTRIM(RTRIM(ISNULL(STUFF((      
                                    SELECT ', ' + CAST(notafiscal.NUM_NOTA_FISCAL AS VARCHAR(15))      
                                    FROM (      
                                                SELECT DISTINCT notafiscal.NUM_NOTA_FISCAL      
                                                FROM notafiscal AS notafiscal WITH(NOLOCK)      
                                                WHERE concat(cast(MONFCL.NR_ENTRADA as char) , MONFCL.SIGLA_FIL) = concat(cast(notafiscal.NR_ENTRADA as CHAR), notafiscal.SIGLA_FIL)    
                                    ) notafiscal      
										ORDER BY notafiscal.NUM_NOTA_FISCAL FOR XML PATH ('')      
											 ), 1, 1, ''), '')))      
								 ) 
				 )  AS NFS  
		) AS MONFCLMNF
