SELECT DISTINCT
	(CASE WHEN MONFCLMNF.STATUS_ENTREGA = 'EM_ATRASO' THEN MONFCLMNF.NUM_DOC end ) AS DOC_ATRASADO
	,(CASE
		WHEN MNFE.NRO_MANIFESTO IS NULL THEN 'SEM VIAGEM'
		WHEN MNFE.DT_CHEGADA IS NULL THEN 'EM VIAGEM'
	ELSE
		'EM TRANSBORDO'
	...
	,MNFE.DT_BAIXA
	,MNFE.FIL_BAIXA_MNF
    ,COALESCE(RDPC.CGC_REDESPACHO,MNFE.CPF_PROPRIETARIO) AS  CPF_CNPJ
	,COALESCE(RDPC.CGC_REDESPACHO,MNFE.CPF_PROPRIETARIO) AS  NOME_PROP
	,(CASE
		...
		( SELECT  DISTINCT
			...
			,MONF.* 
				...
				(
					...
					, (CASE 
						WHEN O.RESPONSAVEL = 'AUTOMATICO' THEN 'MANUAL' 
					...
					WHERE
						...
						and O.CODIGO_OCOR = 
						 ( SELECT MIN(ODM2.CODIGO_OCOR)	FROM tb_solid_ocorrencias AS ODM2
							WHERE
								ODM2. NR_ENTRADA = 	O. NR_ENTRADA
								AND	ODM2.SIGLA_FIL = O.SIGLA_FIL
