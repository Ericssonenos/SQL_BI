	 ...
	 ,(CASE 		
		...
		,MONFCLMNF.* FROM
			(SELECT DISTINCT
				...
				FROM 		
					...
					FROM 	
						...
						, M.* 		
							FROM 	 
							(select dmc.*,  dbo.fn_ret_notas_cte(dmc.NR_ENTRADA,dmc.SIGLA_FIL) notas  from tb_dados_movimento_custo_emis dmc where  	TIPO_DOC = 'CO' AND DT_EMISSAO >= '20200101' AND DT_ENTREGA is  null ) AS M 	
						...
						UNION ALL  		
						SELECT NULL,'NF DE SERVIÇO',0, 'NF DE SERVIÇO', 'NF DE SERVIÇO','NF DE SERVIÇO',  MNF.* ,null FROM tb_dados_movimento_custo_emis AS MNF WHERE TIPO_DOC = 'NF' AND	MNF.DT_EMISSAO >= '20200101' AND DT_ENTREGA is  null
						)MONF 
			...
			MNFE.DT_EMISSAO_MANIFESTO = 
				...
				OR  	
				MNFE.DT_EMISSAO_MANIFESTO IS NULL) total
