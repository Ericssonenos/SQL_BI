...
LEFT JOIN
	...
	ON 	concat(cast(MONFCL.NR_ENTRADA as char) , MONFCL.SIGLA_FIL) = concat(cast(MNF.NR_ENTRADA as CHAR), MNF.SIGLA_FIL)
WHERE 
		MONFCL.DT_ENTREGA is  null
		AND MONFCL.DESCR_TIPO_TRANSP  IN('RODO','SUBCONTRATACAO','ICMS SUBCONT')
		AND MNF.DT_EMISSAO_MANIFESTO = 
			(
				SELECT MAX(MNFDX.DT_EMISSAO_MANIFESTO) FROM tb_dados_manifesto_custo_emis AS MNFDX
				WHERE 
						MNFDX.NR_ENTRADA = 	MONFCL. NR_ENTRADA
					AND	MNFDX.SIGLA_FIL = MONFCL.SIGLA_FIL
			)