...
SELECT DISTINCT 
	TABELA.* 
	,(CASE 			
		WHEN TABELA.DIAS_OCORRENCIA BETWEEN  0 AND 3 THEN '1 A 3'
		WHEN TABELA.DIAS_OCORRENCIA BETWEEN  4 AND 6 THEN '4 A 6' 
		WHEN TABELA.DIAS_OCORRENCIA BETWEEN  7 AND 14 THEN '7 A 14' 	
	...
	FROM	
			...
			,(CASE 
				...
				END) AS RT4
			,(CASE 			
				WHEN DMC02.ESTADO_DEST IN ('DF','GO','MS','MT') THEN 'Centro-Oeste'
				WHEN DMC02.ESTADO_DEST IN ('AL','BA','CE','MA','PB','PE','PI','RN','SE') THEN 'Nordeste' 
				WHEN DMC02.ESTADO_DEST IN ('AC','AM','AP','PA','RO','RR','TO') THEN 'Norte' 
				WHEN DMC02.ESTADO_DEST IN ('ES','MG','RJ','SP') THEN 'Sudeste' 	
				WHEN DMC02.ESTADO_DEST IN ('PR','RS','SC') THEN 'Sul' 		
				END) AS REGIAO
			,(CASE 	
			...
			,(CASE 		
				...
				WHEN DRECE.CPF_PROPRIETARIO IS NOT NULL AND SO03.CODIGO_OCOR IN(104,67)  THEN 'ENTREGANDO'
				WHEN DRECE.CPF_PROPRIETARIO IS NOT NULL AND SO03.CODIGO_OCOR IN(89)  THEN 'CT-E_RETIDO_NO_CLIENTE'
				WHEN DRECE.CPF_PROPRIETARIO IS NOT NULL   THEN 'RETORNO '
			...
			,(CASE 		
				...
				WHEN  DRECE.CPF_PROPRIETARIO IS NOT NULL AND SO03.CODIGO_OCOR IN(104,67)  THEN VCC.PLACA_PRINCIPAL
				WHEN DRECE.CPF_PROPRIETARIO IS NOT NULL AND SO03.CODIGO_OCOR IN(89)  THEN DMC02.DESTINATARIO
				WHEN DRECE.CPF_PROPRIETARIO IS NOT NULL   THEN SO03.DESCRICAO_OCOR
		...
		LEFT JOIN 
			(SELECT * FROM tb_dados_manifesto_custo_emis AS DMFCE01 WHERE DMFCE01.DT_EMISSAO_MANIFESTO = (SELECT MAX(DMFCE02.DT_EMISSAO_MANIFESTO) FROM tb_dados_manifesto_custo_emis AS DMFCE02 WHERE DMFCE02.NR_ENTRADA = DMFCE01.NR_ENTRADA AND DMFCE02.SIGLA_FIL = DMFCE01.SIGLA_FIL ) AND DMFCE01.DT_EMISSAO_MANIFESTO >'20200101') AS DMFCE03 
